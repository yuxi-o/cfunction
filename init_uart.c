...
{
	int uart_fd = 0;
	char tx_buf[BUF_SIZE*2], rx_buf[BUF_SIZE*2], rx_buf_tmp[BUF_SIZE];
...
//					tx_buf_tmp[j++] = (tx_buf[i-2] - '0')*16 + (tx_buf[i-1] - '0');
		...
		// write more times to read
		fds[0].fd = uart_fd;
		fds[0].events = POLLIN;
		for (i = 0; i < 3; i++)
		{
			tcflush(uart_fd, TCIOFLUSH);
			write(uart_fd, txp, len);
			ret = poll(fds, 1, 1000); //1s
			if ((-1 == ret) || (0 == ret))
				continue;
			else 
				break;
		...
		// more times receive error, restart uart
		if((-1 == ret) || (0 == ret))
		{
			mprintf(DRELEASE, E_TIMEOUT, "uart receive timeout !");
			uart_fd = init_uart(argv[1], speed, pserial[0]-'0', pserial[2]-'0', pserial[1]);
			if (uart_fd < 0){
				printf("FUNC:%s, init uart %s failed !\n", __func__, argv[1]);
				exit(-1);
...
//		len = read(uart_fd, rx_buf, sizeof(rx_buf));
		/*read exact nbytes, return when readed rx_buf_len bytes or when there is no byte to read in one second */
		len = dreadn(uart_fd, rx_buf, sizeof(rx_buf), 1);
		tcflush(uart_fd, TCIOFLUSH);
		if(len < 0){
			printf("recevice error, continue...\n");
			continue;
		} 
/*		else if (len < packet_len) // wrong packet length
		{
			// try to receive the last bytes
			mprintf(DRELEASE, E_FRAGMENT, "parse missed packet...");
			for(i=0; i< 3; i++)
			{
				fds[0].fd = uart_fd;
				fds[0].events = POLLIN;
				...
				{
					memset(rx_buf_tmp, '\0', BUF_SIZE);					
					rx_len = dreadn(uart_fd, rx_buf_tmp, sizeof(rx_buf_tmp), 1);
					// check new packet
					ret = 0;
					ret = check_packet_head(rx_buf_tmp, packet_head, head_len);
					if (1 == ret) // new packet
					{
						...
						{
							mprintf(KRELEASE, E_FRAGMENT, "The fragment packet has the correct head, but packet is short!");	
							break;
						}
					}

					for (j = 0; j < rx_len; j++)
						rx_buf[len+j]=rx_buf_tmp[j];
					len += rx_len;
					if (len >= packet_len)
						break;
				}
