#include <stdio.h>

//CRC - 8 x8+x2+x+1
unsigned char crc8(unsigned char *ptr,unsigned char len)
{
    unsigned char crc;
    unsigned char i;

    crc = 0;
    while(len--){
       crc ^= *ptr++;
       for(i = 0;i < 8;i++){
           if(crc & 0x80)
               crc = (crc << 1) ^ 0x07;
           else 
			   crc <<= 1;
       }
    }
    return crc;
}

unsigned char crc_array[256] = {
 
};

unsigned char crc8_table(unsigned char *ptr, unsigned char len)
{
	unsigned char crc = 0;

	while(len-- >0){
		crc = crc_array[crc^*ptr++];	
	}
	return crc;
}

int main(int argc, char *argv[])
{
	unsigned char arr1[]={0x10};
	unsigned char arr2[]={0x10, 0x11};
	unsigned char arr3[]={0x10, 0x11, 0x12};
	unsigned char arr4[]={0x10, 0x11, 0x12, 0x13};
	unsigned char arr5[]={0x10, 0x11, 0x12, 0x13, 0x14};
	unsigned char arr6[]={0x10, 0x11, 0x12, 0x13, 0x14, 0x15};
	
	printf("crc8 arr1 = 0x%x, 0x%x\n", crc8(arr1, 1), crc8_table(arr1, 1));
	printf("crc8 arr2 = 0x%x, 0x%x\n", crc8(arr2, 2), crc8_table(arr2, 2));
	printf("crc8 arr3 = 0x%x, 0x%x\n", crc8(arr3, 3), crc8_table(arr3, 3));
	printf("crc8 arr4 = 0x%x, 0x%x\n", crc8(arr4, 4), crc8_table(arr4, 4));
	printf("crc8 arr5 = 0x%x, 0x%x\n", crc8(arr5, 5), crc8_table(arr5, 5));
	printf("crc8 arr6 = 0x%x, 0x%x\n", crc8(arr6, 6), crc8_table(arr6, 6));

	return 0;
}


